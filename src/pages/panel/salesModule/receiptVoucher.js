...
import Head from 'next/head';
  ...
  const ReceiptVoucher = ({ userEmail, dbVouchers, dbBankAccount, dbCreditSalesInvoice, dbPaymentMethod, dbContacts, dbEmployees, }) => {
      ...
      if(e.target.name === 'journalDate'){
        ...
        if(filteredData.length > 0){
          ...
          setInputList(Array.from({ length: filteredData.length }, () => (
            { id: '', billNo:'' , journalNo, date: journalDate, paidBy:'', balance: 0, paid: 0, netBalance: 0 }
          )))
        }
        else{
          setFilteredData([])
    ...
    const newStateSettings = ()=>{
      ...
      setJournalNo(`RV-${filteredInvoices.length === 0 || !filteredInvoices[filteredInvoices.length - 1].journalNo
        ...
        <div className="fixed inset-0 z-10 overflow-y-auto">
          <div className="flex min-h-full items-stretch justify-center text-center md:items-center md:px-2 lg:px-4">
            <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 translate-y-4 md:translate-y-0 md:scale-95" enterTo="opacity-100 translate-y-0 md:scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 translate-y-0 md:scale-100" leaveTo="opacity-0 translate-y-4 md:translate-y-0 md:scale-95">
              <Dialog.Panel className="flex w-full transform text-left text-base transition md:my-8 md:max-w-2xl md:px-0 lg:max-w-7xl">
                              ...
                              {filteredData.map(( item , index)=>{
                                ...
                                return <tr key={index} className="bg-white flex-col text-black border-b hover:bg-gray-50">
                                  ...
                                  <td className="p-2 text-center min-w-[100px]">
                                    {item.balance ? '' : item.amountReceived ? item.amountReceived.toLocaleString(): 0}
                                  </td>
